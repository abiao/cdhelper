...
OPTIONS
   ...
   -x|--x86_64          File containing list of packages that must be explicitly
      ...
      --unlimitedJCE    Install Unlimited Strength Encryption JCE policy files.
      --singleUserMode  Configure the agent to run in single user (non-root) mode.
      --skipImpala      Don't try and install Impala
      --javaInstallStr  ategy Strategy to use for JDK installation. Valid values are
                        NONE: Do not install JDK
                        AUTO: Install the required versions
      --skipSolr        Don't try and install Solr
      --skipKeyTrustee  Don't try and install KeyTrustee
...
# If requested, install the unlimited strength encryption JCE policy files
install_unlimited_jce()
{
    mark INSTALL_JCE

    if [ "$UNLIMITED_JCE" != "true" ]; then
        echo "Installation not requested. Step will be skipped."
        return
    fi

    # Extract the installation prefix from the packages
    case $PACKAGER in
        apt-get)
            LOC_6=$(action_get "dpkg -L $JAVA_6 | grep \"$JAVA_6_PREFIX\" | sort | head -n 1")
            LOC_7=$(action_get "dpkg -L $JAVA_7 | grep \"$JAVA_7_PREFIX\" | sort | head -n 1")
            ;;
        yum|zypper)
            LOC_6=$(action_get "rpm -ql $JAVA_6 | grep \"$JAVA_6_PREFIX\" | sort | head -n 1")
            LOC_7=$(action_get "rpm -ql $JAVA_7 | grep \"$JAVA_7_PREFIX\" | sort | head -n 1")
            ;;
        *)
            echo unknown packager $PACKAGER, exiting
            exit 1
    ...
    for file in $LOCAL_DIR/*.6; do
        local target=$(basename $file .6)
        local full_target="$LOC_6/jre/lib/security/$target"
        if [ -e "$full_target" ]; then
          echo "Installing unlimited strength $target for Java 6"
        action $SUDO cp $file "$full_target"
        else
          echo "Did not find expected $full_target. Not copying new policy file."
        fi
    done

    for file in $LOCAL_DIR/*.7; do
        local target=$(basename $file .7)
        local full_target="$LOC_7/jre/lib/security/$target"
        if [ -e "$full_target" ]; then
...
{
    mark REPO_REMOVE
    for FILE in $INSTALLED_REPO_FILES $INSTALLED_PRIORITY_FILES; do
        $SUDO rm -f "$FILE"
...
start_agent()
{
    ...
    # OPSAPS-22894
    # We use '4>&-' to explicitly close the lock fd that we use to ensure
    # only one instance of scm_prepare_node is running at a time. If we
    # don't do this, the lock fd will propagate to the Agent script and
    # keep the lock held after scm_prepare_node terminates.
    action $SUDO $SERVICE cloudera-scm-agent $CMD 4>&-
    EXIT_CODE=$?
    echo agent logs:
...
configure_agent()
{
    ...
    if [ $AGENT_USER_MODE == "SINGLE_USER_MODE" ]; then
        action "$SUDO sed -e 's/#\(USER=.*\)/\1/' -i $AGENT_ARGS"
    fi
}
...
# Install package priority override files provided by the server.
install_priority_files()
{
  # If there's no specified priorities file(s), then do nothing
  if [[ ! -d $LOCAL_PRIORITIES ]]; then
    return
  fi

  for file in `find "$LOCAL_PRIORITIES"/*`; do
    echo installing priority file $file
    $SUDO install -m 644 --backup=numbered "$file" "$PRIORITIES_DEST"/
    fail_or_continue $? priority file $file could not be installed
    echo priority file $file installed
    INSTALLED_PRIORITY_FILES="$PRIORITIES_DEST/$(basename $file) $INSTALLED_PRIORITIY_FILES"
  done
}
...
distro_setup_sles()
{
    case $1 in
        11)
            ...
            CHKCONFIG_STOP="off"
            CHKCONFIG_START="on"
            JAVA_6_PREFIX="/usr/java/jdk1.6"
            JAVA_7_PREFIX="/usr/java/jdk1.7"
            JAVA_6="jdk"
            JAVA_7="oracle-j2sdk1.7"
            ;;
        *)
...
distro_setup_debian()
{
    case $1 in
        ...
        precise)
            ...
            ;;
        trusty)
            LOCAL_REPOS=$LOCAL_DIR/repos/ubuntu_trusty
            LOCAL_PRIORITIES=$LOCAL_DIR/priorities/ubuntu_trusty
            ;;
        6.*)
            LOCAL_REPOS=$LOCAL_DIR/repos/debian_squeeze
            ;;
        7.*)
            LOCAL_REPOS=$LOCAL_DIR/repos/debian_wheezy
            ;;
        *)
    ...
    PRIORITIES_DEST=/etc/apt/preferences.d
    PACKAGER=apt-get
    ...
    JAVA_6_PREFIX="/usr/lib/jvm/j"
    JAVA_7_PREFIX="/usr/lib/jvm/j"
    JAVA_6="oracle-j2sdk1.6"
    JAVA_7="oracle-j2sdk1.7"
}
...
parse_arguments()
{
    GETOPT=`getopt -n $0 -o h:,p:,a:,x:,? -l help,host:,packages:,always:,x86_64:,unlimitedJCE:,skipImpala:,javaInstallStrategy:,agentUserMode:,skipSolr:,skipKeyTrustee:,server_version:,server_build: -- "$@"`
    RETVAL=$?
    if [[ $RETVAL -ne 0 ]]; then
        usage
        exit $RETVAL
...
# Setup the list of packages that should never be installed.
setup_blacklist()
{
    ...
    # Don't install Key Trustee if skip option is specified
    if [[ $SKIP_KEYTRUSTEE = "true" ]]; then
        NEVER_INSTALL_PACKAGES="$NEVER_INSTALL_PACKAGES $KEYTRUSTEE_PACKAGE"
    fi

    # Don't install JDK if JAVA_INSTALL_STRATEGY is set to "NONE"
    # By default all the required versions of jdk are installed
    if [[ $JAVA_INSTALL_STRATEGY = "NONE" ]]; then
        NEVER_INSTALL_PACKAGES="$NEVER_INSTALL_PACKAGES $JAVA_6 $JAVA_7"
    fi
...
KEYTRUSTEE_PACKAGE="keytrustee-keyprovider"
UNLIMITED_JCE="false"
AGENT_USER_MODE="ROOT"
SKIP_IMPALA="false"
SKIP_SOLR="false"
SKIP_KEYTRUSTEE="false"
JAVA_INSTALL_STRATEGY="AUTO"
SERVER_VERSION=".*"
SERVER_BUILD=".*"
